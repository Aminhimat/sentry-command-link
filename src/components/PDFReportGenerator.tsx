...
export class PDFReportGenerator {
  ...
  private processImageToPDF(img: HTMLImageElement, x: number, y: number, width: number, height: number, watermarkText?: string): void {
    try {
      ...
      const targetDPI = 72; // Standard screen DPI for maximum speed
      const mmToIn = 1 / 25.4;
      ...
      try {
        ...
        // Fallback to JPEG with maximum compression
      ...
      if (report.image_url) {
        const promise = new Promise<void>(async (resolve) => {
          try {
            ...
            // Compress image for faster PDF generation with maximum compression
