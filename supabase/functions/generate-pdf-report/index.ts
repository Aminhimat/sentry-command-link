...
async function generatePDFWithImages(reports: any[], company: any, reportFilters: any): Promise<Uint8Array> {
  ...
  const transform = { width: 1000, quality: 92, format: 'webp' }  // Target: ~60-80 KB per image
...
async function fetchImageAsBytes(url: string, width = 1000, quality = 92, format = 'webp'): Promise<Uint8Array> {
  // Optimized dimensions + high quality: 1000px @ 92% quality with WebP
  // Target: ~1.8-2.4 MB for 6 pages (30 photos), ~12-16 MB for 58 pages (290 photos)
  let fetchUrl = url
  try {
