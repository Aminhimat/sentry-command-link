...
async function generatePDFWithImages(reports: any[], company: any, reportFilters: any): Promise<Uint8Array> {
  ...
  // Optimized for clear pictures with fast generation
  const totalImages = reportsWithImages.length
  const transform = totalImages <= 20
    ? { width: 1400, quality: 93 }  // High quality, fast processing
    : totalImages <= 100
      ? { width: 1200, quality: 90 }  // Great quality for medium sets
      : { width: 1000, quality: 88 }  // Good quality for large batches
    ...
    if (report.image_url) {
      ...
      if (imgBytes) {
        try {
          ...
          const imgDims = image.scale(0.3) // Maximum quality source with optimal display
...
async function fetchImageAsBytes(url: string, width = 1200, quality = 90): Promise<Uint8Array> {
